...
>(({ className, status, ...props }, ref) => (
  ...
  >
    ...
    {status === 'error' && (
      <CircleXIcon
        className="absolute top-0.5 right-0.5 size-2 text-red-700 stroke-3"
      />
    )}
    {status === 'success' && (
      <CircleCheckIcon
        className="absolute top-0.5 right-0.5 size-2 text-green-700 stroke-3"
      />
    )}
    {status === 'loading' && (
      <LoaderCircleIcon
        className="absolute top-1.5 -right-0.5 size-2 text-blue-700 stroke-3 animate-spin"
      />
    )}
  </div>
));
